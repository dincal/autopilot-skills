...
프로젝트의 **전체적인 룩앤필**을 인터뷰로 다듬습니다.

1. **감사**: 앱을 직접 구동해(가능하면 스크린샷 포함) 현재 구현된 디자인 상태와 비일관성, 디자인 결정이 필요한 예정 기능들을 파악
2. **인터뷰**: 방향·톤 → 비주얼 기초(팔레트 hex, 타이포, 밀도, 다크모드) → 레이아웃·컴포넌트 관례 순으로, 에이전트가 후보 2~3개 + 추천안을 제시하고 사용자가 결정. 세션에 Claude Design 등 디자인 도구가 있으면 실제 목업으로 시안을 만들어 반복하고, 사용자가 목업·링크를 제공하면 최우선 반영
3. **기록**: 합의된 방향을 `design.md`의 **Style Guide 섹션**(살아있는 룩앤필 계약, 제자리 갱신)으로 작성 + Decisions 로그에 날짜 엔트리 추가. 기존 UI가 새 가이드를 위반하는 부분은 리스타일 todo로 제안(수락한 것만 추가)

이후 개발 루프는 모든 UI 기능의 Goal Prompt에 Style Guide 규칙을 제약으로 복사하므로, **기능이 몇 개가 쌓여도 디자인 일관성이 유지됩니다**. 가이드가 답하지 못하는 새로운 디자인 질문만 사용자에게 올라옵니다.
...
4. **루프는 조용히 죽지 않음** — Stop 훅이 런 활성 중 턴 종료를 차단하고 루프로 되돌립니다. 정당한 종료는 Run end 완료(`idle`) 또는 명시적 일시정지(`paused`)뿐. 같은 지점에서 3회 진전이 없으면 안전밸브가 정지를 허용하고, 다음 실행에서 Resume을 제안합니다.
