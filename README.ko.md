...
- 리뷰는 approve 편향: 실제 결함만 차단, 트집으로 머지를 막지 않음
...
| `design.md` | O | UI/UX 디자인 결정 (날짜순 append-only) |
...
7. **리뷰는 approve 편향** — 차단은 화이트리스트 5종(버그/보안·데이터손실/핵심 경로 테스트 부재/인수 기준 미충족/기존 기능 파괴)만. 불확실하면 non-blocking. 스타일 트집으로 머지를 막을 수 없습니다.
