...
- 갭 분석: **검증이 필요한 기준에만** 병렬 파인더 → 중복 병합 (기준 원장이 met으로 기록한 것은 스킵, 필요 기준 0이면 워크플로우 생략)
- 플래닝: 관점이 다른 독립 플랜 2~3개 → 저지 채점 → 승자 합성
- **기능 개발**: 오케스트레이터가 **피처별 최적 워크플로우를 직접 설계** — fan-out/fan-in·파이프라인·저지 패널·검증 스테이지 자유 조합, 단일 에이전트가 최적이면 그것도 답. 하드 제약 6종만 강제: 피처 worktree 안에서만 / 같은 워킹트리에서 동일 파일 동시 편집 금지(소유권 분할 또는 에이전트별 격리 브랜치 + fan-in 머지로 중복 허용) / 전체 테스트+커버리지 그린 / 표준 WORK SUMMARY 산출 / push·gh·`.autopilot` 쓰기 금지 / Workflow 부재 시 표준 폴백
