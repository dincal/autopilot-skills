...
- **기능 개발**: 플랜이 독립 서브태스크 3개 이상으로 분해되면 fan-out/fan-in 워크플로우로 병렬 구현 — 서브에이전트마다 **서로소 파일 소유권**(같은 worktree, 공유 파일 편집 금지), 통합 스테이지가 인터페이스 조정·전체 테스트·커버리지를 전담. 그 미만이면 단일 에이전트가 기본
