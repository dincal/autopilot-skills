...
6. Dev server auto-start: when `devRun.autoStart` (default true) and the project has a runnable dev surface (skip silently for libraries), check `.autopilot/dev-run.json` — if a dev-run is already alive, leave it untouched. Otherwise start one following the `autopilot-dev-run` skill's start protocol (dev-run.json with `"startedBy": "autopilot"`, `dev-run.sh restart`, readiness check) and report the url. Failure to start is non-fatal: warn and continue. The plugin's merge/pull hook keeps it serving newly merged features; leave it running at run end and mention `/autopilot-dev-run stop` in the final report.
