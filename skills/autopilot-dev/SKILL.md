...
6. Dev server auto-start: when `devRun.autoStart` (default true) and the project has a runnable dev surface (skip silently for libraries), check `.autopilot/dev-run.json` — if a dev-run is already alive, leave it untouched. Otherwise start one following the `autopilot-dev-run` skill's start protocol: a session background shell (`run_in_background: true`, `echo DEVRUN_PID=$$ && exec <cmd>`), dev-run.json with `"startedBy": "autopilot"`, readiness check — and report the url. Failure to start is non-fatal: warn and continue. After merges/pulls the plugin hook injects a restart instruction — follow it immediately when it appears; if the server's background task exits mid-run, inspect the output tail and restart it. Leave it running at run end and mention `/autopilot-dev-run stop` in the final report.
...
- **C. Develop** — per feature: create branch + worktree, commit the branch doc onto the new feature branch as its first commit (before spawning any agent), then spawn a background `autopilot:feature-dev` agent; harvest WORK SUMMARY blocks as agents finish. In `single-feature` mode: create the feature branch but develop directly in this session (no worktree, no background agent) — tests included.
...
- **E. Merge & Close** — merge feature PRs into the run branch per `approvals.merge` (sequential; rebase remaining branches after each merge — each merge also brings that feature's branch doc onto the run branch), update todo.md / CHANGELOG.md / CLAUDE.md snapshot on the run branch, keep the run PR (run branch → base) current, remove worktrees.
