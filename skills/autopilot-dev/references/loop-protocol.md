...
2. **Design check**: for any user-facing feature, FIRST consult `design.md`'s `## Style Guide` — when it answers the question, apply it WITHOUT asking. **Whatever governs this feature's look — the applicable Style Guide rules, and (when a mockup is chosen below) that mockup's concrete visual spec — MUST be written into the feature's Goal Prompt as BINDING design constraints**, in enough textual detail to build to WITHOUT seeing the rendered image: layout, components, spacing/sizing, color & type tokens, interaction states, and copy — plus the `design.md` `## Decisions` link and the mockup's source-file paths. The feature-dev agent cannot open a Claude Design render; this text and `design.md` are all it gets, so a design left only in a preview image is a design the developer will ignore. These constraints ride into the FEATURE INPUT `## DESIGN` section (schemas.md). For genuinely new design questions the guide doesn't answer, design tooling is **Claude Design ONLY** (the claude-design MCP; never other design tools):
