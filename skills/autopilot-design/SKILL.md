...
**Design tool integration**: if a design tool is available in this session — the Claude Design MCP (user setup: `claude mcp add --scope user --transport http claude-design https://api.anthropic.com/v1/design/mcp`, then `/design-login`; exposes design-sync and design-creation tools), the built-in DesignSync tool, a design-tool MCP such as Figma or Adobe, or artifact-based mockups — offer to produce visual candidates there and iterate with the user on real mockups instead of descriptions. If none is available, mention the Claude Design setup once and continue without it. The user may also paste links or supply their own mockups — treat those as the strongest signal. If no tooling is available, keep candidates concrete anyway: hex palettes, type scales, named reference apps.
