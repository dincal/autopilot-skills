...
| `ultracode` | `false` | Multi-agent Workflow orchestration inside the loop: fan-out gap analysis, multi-plan judging, dynamically authored development workflows (the orchestrator designs the optimal orchestration per feature — fan-out/fan-in, judge panels, or a single agent — under six hard safety constraints), adversarially verified review. Needs the Workflow tool (falls back gracefully); costs significantly more tokens |
